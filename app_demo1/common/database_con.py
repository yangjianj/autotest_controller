...
class DataManager():
    ...
    def save_api_case(self,apiresult):
        ...
        try:
            table = self._cc.execute("insert into api_case_result (caseid,version,apilink,request_data,result,spend.start_time,end_time) \
            values(_caseid,_version,_apilink,_request,_data,_result,_spend,_start_time,_end_time)")
            self._conn.commit()
            return True
        except Exception as e:
            print(e)
